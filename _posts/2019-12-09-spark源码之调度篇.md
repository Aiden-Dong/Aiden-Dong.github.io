...
![image.png]({{ site.url }}/assets/spark_scheduler_1_1.png)
...
![image.png]({{ site.url }}/assets/spark_scheduler_1_2.png)
...
![image.png]({{ site.url }}/assets/spark_scheduler_1_3.png)
...
`DAGScheduler` 构建 `Stage` 的过程是通过递归调用的。 第一步先创建最后一个 `Stage` 即 `ResultStage` 
...
```
使用的RDD 是我们这个job最后的那个 RDD, 即上面例子中的 `resultRdd`. 然后边通过 `submitStage` 函数将这个stage 提交。

`submitStage`是一个递归划分stage的过程， 从`ResultStage`的 rdd 开始向上递归解析， 直到划分完所有的stage并将所有没有上游依赖的stage提交。
    ...
    if (!visited(rdd)) {
      ...
      if (rddHasUncachedPartitions) {
        for (dep <- rdd.dependencies) {
          dep match {
            case shufDep: ShuffleDependency[_, _, _] =>  // 宽依赖
...
基本上`ShuffledRDD` 的 dependencies 属于 `ShuffleDependency`, 其他的属于`NarrowDependency`派生类的 dependencies.

![image.png]({{ site.url }}/assets/spark_scheduler_1_4.png)

这样就完成了 stage 的分割动作，并将没有依赖关系的Stage提交出去。
...
当将 stage 转化成 task 集合后，便可以将 task 转交给 TaskScheduler 去处理。
...
TaskScheduler 主要负责 task 调度信息的维护。当有资源是选择合适的 task 去调度。
它的实现主要借助于 `Pool` 与 `TaskSetManager` 。

`SchedulableBuild` 主要用于构建调度池 Pool, Pool 会根据调度策略不同会按照不同顺序选出所有的可调度的 TaskSetManager 去调度执行。

调度策略主要分为 `FIFOSchedulingAlgorithm`(使用 `FIFOSchedulableBuilder` 实现调度池) 与 `FairSchedulingAlgorithm`(使用 `FairSchedulableBuilder` 实现调度池).

FIFOSchedulableBuilder 实现的调度池模型大致为:

![image.png]({{ site.url }}/assets/spark_scheduler_1_5.png)


FairSchedulableBuilder 实现的调度池模型大致为:

![image.png]({{ site.url }}/assets/spark_scheduler_1_6.png)

可以看出， `FairSchedulerBuilder` 构造的调度池嵌套多个子调度池， 每个调度池的权重影响到调度池内的 `TaskSetManager` 的调度时机。
...
![image.png]({{ site.url }}/assets/spark_scheduler_1_7.png)
