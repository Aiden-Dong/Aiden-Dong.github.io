...
![image.png](https://upload-images.jianshu.io/upload_images/10402860-18de35f091665aa1.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![image.png](https://upload-images.jianshu.io/upload_images/10402860-77e52e6116555d8c.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

首先，我需要插入一个关键字：30，可以得到如下的结果：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-a6ff655b59aaa101.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

再插入26，得到如下的结果：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-8228ad0f2e9b8603.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![image.png](https://upload-images.jianshu.io/upload_images/10402860-eff5a6e899491196.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

OK，然后我们继续插入新的关键字：85，得到如下图结果：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-f9657fa04a12d79f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)


正如图所示，我需要对刚才插入的那个结点进行“分裂”操作，操作方式和之前的一样，得到的结果如下：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-2324c340099eaf0f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)


哦，当我们分裂完后，突然发现之前的那个结点的父亲结点的度为4了，说明它的关键字数超过了m-1，所以需要对其父结点进行“分裂”操作，得到如下的结果：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-bcf284a8433f1ffc.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)


好，我们继续插入一个新的关键字：7，得到如下结果：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-17bc659bea76fbbb.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)


哦，当我们分裂完后，突然发现之前的那个结点的父亲结点的度为4了，说明它的关键字数超过了m-1，所以需要对其父结点进行“分裂”操作，得到如下的结果：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-2e5285d5c2c87a7e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

到了这里，我就需要继续对我们的父亲结点进行分裂操作，因为它的关键字数超过了：m-1.

![image.png](https://upload-images.jianshu.io/upload_images/10402860-415d81b849d57725.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

哦，终于遇到这种情况了，我们的根结点出现了关键子数量超过m-1的情况了，这个时候我们需要对父亲结点进行分列操作，但是根结点没父亲啊，所以我们需要重新创建根结点了。

![image.png](https://upload-images.jianshu.io/upload_images/10402860-c31c452e8916daa0.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![image.png](https://upload-images.jianshu.io/upload_images/10402860-a1f56ed8b5d447eb.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

删除12后的结果如下，只是简单的删除关键字12和其对应的指针。

![image.png](https://upload-images.jianshu.io/upload_images/10402860-cb478e786493264e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

**第二种：关键字个数n等于( 上取整）[ m/2 ]-1，而且该结点相邻的右兄弟(或左兄弟)结点中的关键字数目大于( 上取整）[m/2]-1.**

![image.png](https://upload-images.jianshu.io/upload_images/10402860-f38332cb21849315.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

如上图，所示，我们需要删除50这个关键字，所以我们需要把50的右兄弟中最小的关键字：61上移到其父结点，然后替换小于61的关键字53的位置，53则放至50的结点中。然后，我们可以得到如下的结果：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-34146d0ac6bae580.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

**第三种：关键字个数n等于( 上取整）[ m/2 ]-1，而且被删关键字所在结点和其相邻的兄弟结点中的关键字数目均等于（上取整）[m/2]-1**

![image.png](https://upload-images.jianshu.io/upload_images/10402860-0935851058c16e74.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

如上图所示，我们需要删除53，那么我们就要把53所在的结点其他关键字（这里没有其他关键字了）和父亲结点的61这个关键字一起合并到70这个关键字所占的结点。得到如下所示的结果：

![image.png](https://upload-images.jianshu.io/upload_images/10402860-0ead5513c7587822.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
