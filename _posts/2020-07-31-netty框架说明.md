...
channel 作为 netty 的核心对象之一， 其内部主要封装了用于实现连接与通信的主要属性与方法，
它是我们使用 netty 的核心操作对象。
...
右侧的 `channel` 模块主要负责维护 socket 通信接口， 左侧的 `pipeline` 主要维护负责对于通信事件的逻辑处理。
